...
#include "./include/SubstrateDebug.hpp"
...
#include "./include/SubstrateARM.hpp"
...
if (false) fail:
		...
		for (unsigned offset(0); offset != used / sizeof(uint16_t); ++offset)
		{
			...
			//	*--trailer =  (reinterpret_cast<uint32_t>(area + offset) + 4 + jump) /*& 0xFFFFFFFC*/ ;
...
static int checkJumpFunc(void* funcAddr){
	//HLOGI("function address:%08x  val:[%08x]\n", funcAddr, *(unsigned int*)funcAddr);
	int state = 0;
	int width = 0;
	while(1){
		hde64s decode;
		memset(&decode, '\0', sizeof(hde64s));
		hde64_disasm((char*)funcAddr + width, &decode);
		if(decode.opcode == 0x8B && decode.modrm == 0x1C && decode.sib == 0x24){
			unsigned int val = *(unsigned int*)((char*)funcAddr + width);
			//HLOGI("val:%08x\n", val);
			if(val == 0xC3241C8B){
				return 1;
			}
		}
		if(decode.opcode == 0xC3){
			//HLOGI("over decode.opcode = 0xC3\n");
			break;
		}
		width += decode.len;
	}
				...
				if ((decode.modrm & 0xc7) == 0x05)
				{
					...
					if (backup[offset] == 0xe8)
					{
						...
						else
						{
		                	//HLOGI("relative:%08x\n", relative);
		                	void *destiny(area + offset + decode.len + relative);
		                	int state = checkJumpFunc(destiny);
		                	if(state == 1){
		                		MSWrite<uint8_t>(current, 0xBB);
		                		void *addr(area + offset + decode.len);
		                		uint32_t addr_val =(uint32_t) addr;
		                		MSWrite<uint32_t>(current, addr_val);
		                	}else{
		                    	...
		                    	//HLOGI("MSSizeOfJump(destiny, current + MSSizeOfSkip()):[%08x]\n", MSSizeOfJump(destiny, current + MSSizeOfSkip()));
