...
static int ParseResourceChunk(Parser_t *ap)
{
	uint32_t chunkSize;
...
// open the xml and initialize ap
...
AxmlEvent_t AxmlNext(void *axml)
{
	...
	if(chunkType == CHUNK_STARTTAG)
	{
		...
		if (flag)
		{
			...
			for(i = 0; i < attr->count; i++)
			{
				attrname = AxmlGetAttrName(ap, i);
				if (!strcmp(attrname, "name"))
				{
					g_appAttIndex = i;
					g_packflag = 1;
					AxmlGetAttrValue(ap, i);
				}
				if (!strcmp(attrname, "theme"))
				{
					g_namePosIndex++;
				}
	...
	{
		fprintf(stderr, "Error: chunkType.\n");
		event = AE_ERROR;
	}
...
static char * GetString(Parser_t *ap, uint32_t id)
{
	...
	if (g_packflag == 1)
	{
		g_packNameLen = (size_t)chNum*2;
		memset(g_packName, 0x0, 400);
		memcpy(g_packName, offset+2, g_packNameLen);
		g_packflag = 2;
	}
...
static int InitBuff(Buff_t *buf)
{
	if(buf == NULL)
		return -1;
	buf->size = 32*1024;
...
static int PrintToBuff(Buff_t *buf, size_t maxlen, const char *format, ...)
{
	va_list ap;
	size_t len;
