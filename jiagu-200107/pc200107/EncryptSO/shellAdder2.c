...
//  char funcName[] = "Java_com_thomas_crackmeso_MainActivity_verify";
  ...
  // {
    ...
    encContent = (char*) malloc(info.st_size);
    if(content == NULL){
    ...
    rc4_crypt(sTemp, (unsigned char *)content, info.st_size, (unsigned char *)encContent, 0);

    lseek(fd, info.st_value - 1, SEEK_SET);
    if(write(fd, encContent, info.st_size) != info.st_size){
      puts("Write modified content to .so failed");
      goto _error;
    }
    puts("Complete encrypt libsec.so!");
  // }
