...
namespace GodinHook {
  ...
  public:
    HookModule(string name,size_t base_addr):module_name_(name),base_addr_(base_addr),
                                             ...
                                             sym_ptr_(NULL),sym_size_(0),gnu_symndx_(0),
   ...
   void setIsRegistered(bool value){this->isRegistered = value;}
  ...
  private:
   ...
   bool externalLookup(char const* symbol, ELFW(Sym) **sym, int *symidx);
